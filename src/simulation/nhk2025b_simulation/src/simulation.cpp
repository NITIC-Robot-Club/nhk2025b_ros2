...
void simulation::timer_callback () {
    if (count_ != 0 && sig_) {
        ...
        float angle    = std::atan2 (y_sum_ / count_, x_sum_ / count_) + z_;
        float distance = std::hypot (x_sum_ / count_, y_sum_ / count_);
...
void simulation::swerve_callback (const nhk2025b_msgs::msg::Swerve::SharedPtr msg) {
    ...
    for (int i = 0; i < 4; ++i) {
        float theta = msg->wheel_angle[i];
        float v     = msg->wheel_speed[i] / 60.0f * 2.0f * M_PI * wheel_radius;
