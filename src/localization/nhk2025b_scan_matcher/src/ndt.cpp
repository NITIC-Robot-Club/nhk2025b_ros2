...
ndt::ndt (const rclcpp::NodeOptions& options) : Node ("ndte", options) {
    ...
    map_sub_ =
        this->create_subscription<nav_msgs::msg::OccupancyGrid> ("/behavior/map", 10, std::bind (&ndt::mapCallback, this, std::placeholders::_1));
...
void ndt::scanCallback (const sensor_msgs::msg::LaserScan::SharedPtr scan_msg) {
    ...
    RCLCPP_INFO (
        this->get_logger (), "x: %f, y: %f, theta: %f", result_pose.pose.position.x, result_pose.pose.position.y,
        tf2::getYaw (result_pose.pose.orientation));
}
