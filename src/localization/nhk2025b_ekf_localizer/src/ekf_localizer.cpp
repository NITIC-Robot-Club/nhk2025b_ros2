...
: Node("ekf_localizer", options), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_) {
    ...
    fused_pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
        "/localization/current_pose", 10);

    imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
        "/sensor/imu", 50, std::bind(&ekf_localizer::imu_callback, this, std::placeholders::_1));

    odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/localization/wheel_odometory", 20,
        std::bind(&ekf_localizer::odom_callback, this, std::placeholders::_1));
...
void ekf_localizer::predict(const nav_msgs::msg::Odometry& odom,
                            const sensor_msgs::msg::Imu& imu, double dt) {
